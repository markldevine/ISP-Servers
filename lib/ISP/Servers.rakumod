            ...
            if $out {
                ...
                for @dsm-sys -> $rcd {
                    if $rcd ~~ m:i/ ^ SERVERNAME \s+ $<client>=(<alnum>+?) '_' $<server>=(<alnum>+) \s* $ / {           # %%% make this accept client names with '_'; take all but not the last '_'
                        ...
                        %!isp-servers{$current-server}{$current-client} = ISP-SERVER-INFO.new(:SERVERNAME($current-client ~ '_' ~ $current-server)) if %!isp-servers{$current-server}:exists;
                    }
                    elsif $rcd ~~ m:i/ ^ \s* TCPS\w* \s+ $<value>=(.+) \s* $/ {
                        %!isp-servers{$current-server}{$current-client}.TCPSERVERADDRESS = $/<value>.Str if %!isp-servers{$current-server}:exists;
                    }
                }
                return self;
            }
